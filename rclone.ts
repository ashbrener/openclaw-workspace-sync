/**
 * rclone infrastructure for workspace cloud sync.
 *
 * Self-contained — no imports from openclaw core.
...
function runExec(
  ...
  const { timeoutMs = 10_000, maxBuffer = 1_000_000 } = opts;
  return new Promise((resolve, reject) => {
    execFile(command, args, { timeout: timeoutMs, maxBuffer }, (err, stdout, stderr) => {
      if (err) {
...
export async function ensureRcloneInstalled(
  ...
  if (isMac) {
    const hasBrew = await runExec("which", ["brew"]).then(
      () => true,
      () => false,
    );
  ...
  if (isLinux || isMac) {
    ...
    try {
      ...
      await runExec("sudo", ["bash", "-c", stdout], { timeoutMs: 120_000 });
      cachedRcloneBinary = null;
      return true;
    } catch (err) {
      pluginLogger?.error(
      ...
      pluginLogger?.error("Try installing manually: https://rclone.org/install/");
      return false;
    }
  }

  pluginLogger?.error("rclone not found. Please install manually: https://rclone.org/install/");
  return false;
}
...
export function getDefaultRcloneConfigPath(stateDir?: string): string {
  const base = stateDir ?? process.env.OPENCLAW_STATE_DIR ?? join(homedir(), ".openclaw");
...
export function ensureRcloneConfigFromConfig(
  ...
  if (isRcloneConfigured(configPath, remoteName)) return true;
  if (!syncConfig?.provider || syncConfig.provider === "off") return false;
...
export async function authorizeRclone(
  ...
  if (appKey && appSecret) args.push(appKey, appSecret);
  ...
  try {
    ...
    if (tokenMatch) return { ok: true, token: tokenMatch[0] };

    const jsonMatch = combined.match(
      /Paste the following into your remote machine[\s\S]*?(\{[\s\S]*?\})\s*$/m,
    );
    if (jsonMatch) return { ok: true, token: jsonMatch[1] };
...
export async function runBisync(params: {
  ...
  if (params.copySymlinks) args.push("--copy-links");
  if (params.resync) args.push("--resync");
